...
## Day 18 — Round 6 polish: SAS in About + license honest + confetti above the fold (commit `4cf6dd9`, build `ddcf88ba`)
Three Round 6 polish items from on-Seeker review. (B) AboutScreen TECH STACK gained a "Solana Attestation Service" entry linking to attest.solana.com — Round 5 shipped the SAS infrastructure but the credits screen didn't acknowledge it; judges who tap (i) on CardReveal/Gallery now see SAS in the stack alongside Solana/Helius/Pinata/Gemini/Bubblegum. (C) Confetti pieces clamped to the top 40% of viewport (was 0–100%). The static-particle confetti was covering "Your story is on-chain", "cNFT minted to …", and the SAS "Verified via SAS" pill below; single-constant change in `Confetti.tsx` (`TOP_RANGE_PCT = 40`), no animation rework. Title area still gets the celebratory burst; copy below is fully readable. (D) "Open source · MIT planned" → "Open source · license TBD" — no LICENSE file in repo, so honest framing replaces the soft promise. Issue E (Diamond vs Paper Hand preset rate-limit) deferred per Vow's prior decision; custom-wallet path demos the same flow. APK uploaded in place to v0.1.0-preview (72,495,115 bytes, +108 vs Round 5 — only label string changes); pre-flight upload protocol followed (download → upload --clobber → release-view confirm → curl public URL match).

## Day 17 — Round 5: SAS attestations + Seeker Genesis (commits `71f2884`, `5dc94a8`, build `5d163b62`)
Lifts WRAP from "consumer identity app" → "identity infrastructure with consumer reference app" via two parallel integrations. **Phase 1 (SAS) — complete:** sas-lib + @solana/kit installed, but @solana/kit's crypto.subtle dependency makes RN-side issuance infeasible without a non-trivial polyfill. Decision: keep all kit-heavy code in Node, ship 5 real on-chain demo attestations (Toly/Raj/Mert/Ansem/Sample) on devnet, RN client renders "Verified via SAS" pill via static-map lookup (no runtime crypto). Issuer setup script `scripts/sas-issuer-setup.ts` is idempotent — creates the WRAP_IDENTITY_V1 schema (`4BAEmFLZaQE2QZAAeB64wTuLgXrWu1s1a98Wi9Aobji9`) and Credential PDA (`EUoMouVtQJFhaqf3aCErWCa3gkQ9QDawjeMfe33aiwKg`) under issuer `6uRTvYnEWJNmDayu7unoyTjqCRyuENwWVUyEDjbbV8Wx`, then issues attestations. Schema fields: `wrap_score u8`, `og_percentile u8`, `wallet_age_days u32`, `card_type String`, `issued_at u32`, `wrap_version String`. Verification proof-of-concept via `scripts/sas-verify.ts <pubkey>` round-trips Toly's attestation cleanly: `{ wrap_score: 98, og_percentile: 1, card_type: 'OG_STATUS', wallet_age_days: 1700 }`. Sample tx: `2Npi6Sq2NtfNvh1C8iJRUbJzyi2edvsF8NQhqhRimk8ZEXKjQaDUcaCFVyWZk4d6XjvkqKa2tqR5mH158XZzq5Bb`. UI: SAS pill on MintConfirm + CardReveal carousel; tap opens Solscan with attestation PDA. README has new "Identity Infrastructure (SAS)" section with TS SDK snippet, 3 PDAs as Solscan links, schema layout, and verify command. Live RN-side issuance deferred to Phase 1.5 (post-Frontier). **Phase 2 (Seeker Genesis) — placeholder:** Hermes Genesis mint research still in progress at build time; `services/seeker/genesis.ts` shipped with `GENESIS_TOKEN_MINT = ''` short-circuit (returns `holds: false` without RPC) and a clear `TODO(genesis-mint)` marker. Architecture wired end-to-end: 15-min AsyncStorage cache, Helius mainnet RPC `getTokenAccountsByOwner` filtered by mint, graceful failure → `holds: false`. Badge in 3 surfaces: CardReveal pill ("🎟️ SEEKER OG"), BattleInput WALLET A row tag ("⚡ SEEKER OG"), Onboarding post-connect toast ("Welcome, Seeker OG ✨"). CRITICAL: Genesis status NOT passed to scoring engine — pay-to-win perception kills WRAP's narrative. One-line swap: edit constant when real mint lands, all 3 surfaces light up. README "Built for Seeker" section added documenting MWA, Seed Vault, Genesis detection, and Solana dApp Store roadmap. APK rebuilt (`5d163b62`, https://expo.dev/accounts/vowctminibro/projects/wrap/builds/5d163b62-54da-4595-8747-2f7ffa51a8a0), uploaded in place to v0.1.0-preview (72,495,007 bytes, +5,740 vs Round 4.5). Pre-flight upload protocol followed — release-view confirm + curl public URL byte-match. WRAP now ships as identity infrastructure (SAS) with reference consumer app (cards/battle/leaderboard) + Solana Mobile native (Genesis, MWA).

## Day 16 — Round 4.5: 4 leftover Round 4 issues fixed (commit `7f29f1c`, build `921c4f58`)
Vow's on-Seeker test of the Round 4 APK surfaced 4 leftover issues, all fixed in one rebuild. (A) **Onboarding "Leaderboard" CTA cropped** — Round 4's Mark logo addition (`596bf85`) pushed content past ~720dp viewports. Wrapped SafeAreaView content in a ScrollView with `flexGrow:1 + justifyContent:'space-between'` contentContainer. Tall screens unchanged; short screens scroll the leaderboard CTA into view. (B) **Leaderboard ↗ share button "tap does nothing"** — root cause was UX, not logic: capture+upload took 3-5s silent, then Android's https-URL share-sheet rejection fell through to a 2s `ToastAndroid.SHORT` "Link copied" that judges' eyes had already left. Fixes: instrumented every step with `[leaderboard:share]` console logs; switched toasts to `LONG`; added immediate "Generating share image…" feedback on tap; success path now uses durable `Alert.alert('Leaderboard link copied', publicUrl)` modal; Pinata-failure fallback opens getwrap.vercel.app via `Linking.openURL` so the tap always produces a visible action. (C) **BattleResult REPLAY badge** — was a static `<View>`, judges tapped expecting animation restart. Converted to `<Pressable>` threaded through the Header sub-component via new `onReplay` prop that calls `setRevealedRounds(0); setSkipped(false)` — re-runs the existing animation effect from cached state, no engine refetch. (D) **Live battle scores collapsing near zero** — `battle-engine.ts:244` called `getWalletTransactions(address)` with default limit=100, but Toly/Ansem post ~100 txs/day so the window only reached ~1 day back: walletAgeDays=14 → og_status=14/90=0.16 (Vow saw 0.2/0.2); both wallets hit the cap → totalTransactions=100 → volume=log10(100)×2.5=exactly 5.0/5.0 (the matching tie). Bumped limit 100 → 1000 in the battle path; helius.ts retry layer absorbs the latency. APK uploaded in place to v0.1.0-preview (72,489,267 bytes, updated 2026-05-04T13:05:13Z); pre-flight upload protocol followed (download → upload --clobber → release-view confirm → curl public URL match).
